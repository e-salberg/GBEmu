...
const char* cartridge_licensee_name()
{
    ...
    else if (ctx.header->old_licensee_code == 0x33)
    {
...
{   
    ...
    printf("\t Checksum      : %2.2X (%s)\n", ctx.header->header_checksum, (checksum & 0xFF) == (ctx.header->header_checksum & 0xFF) ? "PASSED" : "FAILED");
